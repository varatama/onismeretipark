...
export default function ExperiencePlayerPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        async function loadExperience() {
            ...
            try {
                // If not logged in, increment trial views
                if (!user) {
                    const { incrementTrialViews } = await import('@/lib/trial');
                    incrementTrialViews();
                }

                const [exp, stps] = await Promise.all([
                ...
                // If logged in, check premium and load progress
                if (user) {
                    ...
                    // Safety gating for authenticated but non-premium
    ...
    const handleNext = async () => {
        ...
        // If trial user (no user object), just move locally
        if (!user) {
            if (isCompleted) {
    ...
    return (
        <TrialGuard experienceId={id || undefined}>
